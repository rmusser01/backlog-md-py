...
| Rich Markdown editing | Required for full clone | Safe rendering, Mermaid detail diagrams, Markdown toolbar, server-rendered preview, and owned-section replacement implemented | Task detail Markdown is rendered through a safe HTML renderer, Mermaid fences are exposed to the client-side Mermaid renderer with strict security settings, and the browser edit form can replace raw Markdown Implementation Notes and Final Summary sections through the existing parser-preserving writer. Description, Implementation Notes, and Final Summary textareas expose local Markdown formatting controls plus Edit/Preview mode backed by the same safe server renderer. Full WYSIWYG editing remains deferred. |
...
- End-to-end browser tests for rich edit flows beyond the
  ...
  edit toolbar, safe server-rendered preview, archive
