...
- Intended next release: `v0.3.0`.
- Release commit must be on `origin/main` after the release-prep PR merges.
- Do not reuse `v0.2.0`: that tag already exists, and PyPI package versions are
  immutable after publication.
- Keep the package in beta unless `pyproject.toml`, `docs/stability-policy.md`,
  and the changelog are intentionally updated for a different support contract.
...
git tag -a v0.3.0 -m "backlog-md-py v0.3.0"
...
```bash
gh release view v0.3.0 --repo rmusser01/backlog-md-py
...
/tmp/backlog-md-py-pypi-smoke/bin/python -m pip install "backlog-md-py==0.3.0"
