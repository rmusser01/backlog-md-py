...
backlog-py --cwd /path/to/project task create "Implementation task" --plan "1. Inspect current code." -a codex -l implementation --priority high --ac "Behavior covered" --dod "Tests pass" --dep 1 --plain
backlog-py --cwd /path/to/project task edit TASK-2 --plan "1. Patch focused scope." --append-plan "2. Verify behavior." -a reviewer -l ready --priority medium --notes "Implementation details." --ac "Regression covered" --dod "Package check passes" --remove-ac 1 --append-final-summary "Ready for review." --plain
...
print(task_edit(project, "task-1", planSet="1. Patch focused scope.", planAppend=["2. Verify behavior."], assignee=["reviewer"], labels=["ready"], priority="medium", notes="Implementation details.", finalSummaryAppend=["Ready for review."]))
