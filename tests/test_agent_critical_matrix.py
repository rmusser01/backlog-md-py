...
def test_agent_critical_inventory_tracks_mcp_task_edit_clear_priority_surface():
    inventory = load_builtin_inventory()
    by_name = {item.name: item for item in inventory.items}
...
def test_agent_critical_inventory_tracks_mcp_project_status_surface():
    ...
    assert by_name["mcp:project-status"].expected == "project_status(project, recentLimit=5)"


def test_agent_critical_inventory_tracks_mcp_init_required_resource():
    inventory = load_builtin_inventory()
    by_name = {item.name: item for item in inventory.items}

    assert by_name["mcp:init-required"].expected == "backlog://init-required"
...
def test_agent_critical_inventory_tracks_task_status_alias_surface():
    ...
    assert "-s <status>" in by_name["cli:task-list-plain"].expected
